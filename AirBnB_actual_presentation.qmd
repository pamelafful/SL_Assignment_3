...
#| message: false
#| warning: false
#| fig-cap: Non Linear Kernal SVM CV Error vs Cost
#| fig-height: 5
#| fig-width: 5
#| out-width: 80%
...
![Non-linear Kernel SVM CV Error Per Grid Hyperparameter](nl_grid_summ.png){width="387" height="213"}
...
![Neural Network grid search summary](new_grd_summ_nn.png){width="423"}
...
cv_specificity=unname(confusionMatrix(data = df$pred_labels, reference = df$fully_booked_30)$byClass['Specificity'])

cv_sensitivity=unname(confusionMatrix(data = df$pred_labels, reference = df$fully_booked_30)$byClass['Sensitivity'])

cv_balanced_accuracy=unname(confusionMatrix(data = df$pred_labels, reference = df$fully_booked_30)$byClass['Balanced Accuracy'])

cv_f1=unname(confusionMatrix(data = df$pred_labels, reference = df$fully_booked_30)$byClass['F1'])
