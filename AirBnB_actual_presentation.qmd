...
Claire Tours, a UK-based touring company, would like to partner with Airbnbs in Cape Town to sell touring packages. They are conducting research to predict which potential hosts would be ideal candidates to partner with based on their attributes. They thus require predictions of demand/vacancy at these properties. In this case, demand is proxied by the target variable, fully_booked_30, which is a binary variable that takes the value 1 if the Airbnb is fully booked in the next 30 days and 0 otherwise. This is done by training 3 classification models and comparing their performance metrics. The models are:

-   SVM with a linear kernel
-   SVM with a radial kernel
-   Feed-forward Neural Network

The predictions from the best-performing model, which can accurately classify Airbnbs that will not be fully booked in the next 30 days, will serve as input for the company’s potential monthly revenue models. \
\
...
The data is highly imbalanced in favour of the negative class) with \~82% of observations belonging to fully_booked_30=no . Since the modelling aims to predict availability, this will be treated as the positive class in modelling- this is the target class.\
\
There is no clear distinction between the classes spatially with both classes spread out evenly across latitudes and longitudes.
...
-   For the host age (measured by how long the host id has been an Airbnb in months) the median (118 months) host age is \~5% higher for the "yes" class compared to the no class. Moreover, the no class has a longer range of host ages, although the distributions about the median for both groups are similar; most host ages lie below their respective median host age.
...
**\
**Cost Parameter:** This is a constant \>0 controlling how strict the model (alternatively, how wide or narrow the margin is around the linear decision boundary). A higher cost value indicates a stricter model (and thus a narrower margin, with fewer boundary violations), whilst a lower cost indicates a more tolerant model (or wider margin). For the linear kernel SVM, the cost parameter is tuned with the following options:\
...
q=liner_svm_model$performances %>% 
  ...
  annotate(
    ...
    x = 2.3,
    y = 0.183456,
    label = "Model starts to overfit at C >0.9",
    colour = "blue",
    size = 4,
    hjust = 0.5
  ) +
  ...
  labs(
    title = 'Linear Kernel SVM CV Error vs Cost',
    y = 'CV Miscl. Error'
  ) +
...
A radial-kernel SVM was chosen to train on the data. This kernel has an additional similarity parameter, gamma, that controls the rate of (dis)similarity between points based on their distance; thus, it is also tuned during training in addition to the cost parameter:

1.  Cost: Sequence starting from 0.15 to 3, in steps of 0.15, amounting to 20 cost parameter options
2.  Gamma: \[0.05 0.25 0.45 0.65 0.85 1.00\]
...
**Across parameter combinations, the model starts to overfit as the gamma parameter increases past 0.50
  ...
  labs(title='Non Linear SVM Tuning Summary',x='gamma',y='CV Misclass Error')+
   geom_hline(yintercept = min_err, 
     ...
     geom_vline(xintercept = 0.45, 
               ...
               linetype = "dashed", 
               linewidth = 1.1)+
  annotate("text", x = 0.455, y = 0.1850, label = paste(a,', ',b), color = "purple",size = 4)+
...
![Non-linear SVM CV Misc. Error per Grid Hyperparameter Combination](images/clipboard-253748003.png){width="313"}

# Feed Forward Neural Network

**Hyperparameters**

For the feed-forward neural net, three hyperparameters were tuned in the training process:

1.  **Number of hidden layer nodes**: \[2, 5, 10,15\]
2.  **L1 regularization parameter,** which helps hedge the model against overfitting by minimizing (to 0 for some) the weights coming from non-relevant nodes (including the input layer). Values of this control the strength of the penalty, with smaller values indicating a more flexible and tolerant model: \[0.5,0.15,0.25\]
3.  **Number of Training Epochs**- the number of times the (entire) training data feeds through the neural net: \[500, 1000, 1500,2000\], although early stopping is enabled to prevent the model from overfitting.
...
![NN CV Log Loss Per Grid Hyperparameter Combination](new_grd_summ_nn.png){alt="NN CV Log Loss Per Grid Hyperparameter Combination" width="285" height="241"}
...
# thresahold ~0.5 fro comparison
tau_2=0.50

nn_f1_2= h2o.F1(h20_cv_perf)%>%
  filter( round(threshold,2)==0.51) %>% 
  pull(f1) # terrible f1 at this threshold



nn_spec_2=h2o.specificity(h20_cv_perf) %>% 
  filter( round(threshold,3)==0.510) %>% 
  pull(tnr)

nn_sens_2=h2o.sensitivity(h20_cv_perf) %>% 
  filter( round(threshold,1)==0.510) %>% 
  pull(tpr) # terrible
...
nn_bal_acc_2=(nn_sens_2+nn_spec_2)/2

h2o.cross_validation_holdout_predictions(best_model)


# Higher threshhold due to imbalance target
tau=0.8205246
...
# Using 0.5 TS
all_met_cv_05=as.data.frame(h20_cv_perf@metrics$thresholds_and_metric_scores)
all_met_cv_05=all_met_cv_05 %>% 
  filter( round(threshold,2)==0.51)

# Using 0.81
all_met_cv_081=as.data.frame(h20_cv_perf@metrics$thresholds_and_metric_scores)
all_met_cv_081=all_met_cv_081 %>% 
  filter( round(threshold,3)==0.821)
...
-   Both SVMs show perfect sensitivity scores, that is, they are able to perfectly predict all positive labels as positive (target=no); however, they had very poor specificity. The models were not able to correctly predict the majority of the negative class. The F1 scores for both are relatively high due to perfect positive class predictions, whilst the balanced accuracy suffers due to both models' inability to predict the minority class.
-   The NN model is slightly more balanced in its metrics, at different probability class assignment thresholds:
    -    At $\tau = 0.51$, the model was able to correctly classify \~100% the target class (fully_booked=no), with zero correct minority class predictions. The F1 score is also high, mostly bolstered by the high positive class prediction accuracy; the balanced accuracy score is at 50%- the model performs no better than a coin toss (again, mostly due to poor correct minority class predictions).
    -   At $\tau = 0.81$, the model was able to correctly classify \~73% of the minority (specificity) class label and only \~51% of the majority, positive label class. The F1 score decreases to 0.65, although balanced accuracy improves to 65% due to the improvement in correct negative (minority class) predictions.
...
# Extract metrics from threshold = 0.51 table
nn_spec_051 <- all_met_cv_05$specificity
nn_sens_051 <- all_met_cv_05$recall
nn_f1_051   <- all_met_cv_05$f1
nn_bal_051  <- all_met_cv_05$absolute_mcc

# Balanced Accuracy calculation
nn_bal_051 <- (nn_spec_051 + nn_sens_051)/2


# Extract metrics from threshold = 0.821 table
nn_spec_081 <- all_met_cv_081$specificity
nn_sens_081 <- all_met_cv_081$recall
nn_f1_081   <- all_met_cv_081$f1

# Balanced Accuracy calculation
nn_bal_081 <- (nn_spec_081 + nn_sens_081)/2
...
cross_mod_metrics <- data.frame(
  ...
  Model = c(
    'Linear Kernel SVM',
    'Radial Kernel SVM',
    'Feed Forward Neural Net (\u03C4 = 0.51)',
    'Feed Forward Neural Net (\u03C4 = 0.821)'
  ),
